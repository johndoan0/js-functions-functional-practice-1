...
// -----------
//OUTPUT: The value of list[i].propertyName
...
function find(list, predicate) {
    var findname = function(objel){
        if (predicate(objel) === true){
            return objel 
        }
        else return
    }
    // console.log(list.filter(findname))
    return list.reduce(findname)
}
...
function where(list, properties) {
    var filterplays = function(objs){
       for(value in properties){
        if (properties[value] !== objs[value])
            return false
       }
       return objs
    }    
    return list.filter(filterplays)
}
